 ...
 *
 * 10. Contact departments/children
...
export class LarkCalendarProvider implements CalendarProvider {
  ...
  // ---------------------------------------------------------------------------
  // Internal: authenticated HTTP helper.
  // ---------------------------------------------------------------------------
  private async call<T = any>(
    ...
    try {
      ...
      console.error(
        `[lark.http] x ${method} ${path} network-error +${Date.now() - t0}ms: ${String(
    ...
    if (data.code !== 0) {
      console.error(
        `[lark.http] x ${method} ${path} http=${res.status} +${Date.now() - t0}ms code=${
          data.code
  ...
  /**
   * Snapshot the whole directory as DirectoryCandidate[].
   */
  ...
  async resolveUsers(queries: string[]): Promise<ResolvedUser[]> {
    ...
    // Name queries: return full directory as candidates for LLM
    if (nameIdx.length > 0) {
      console.error(
  ...
  async createEvent(input: CreateEventInput): Promise<CreatedEvent> {
    ...
    // Add attendees in a second call
  ...
  async sendTextDM(openId: string, text: string): Promise<string> {
    ...
    try {
      ...
      console.error(
        `[lark.dm] ok DM delivered to ${openId} messageId=${messageId}`,
      );
      return messageId;
    } catch (err) {
      console.error(
        `[lark.dm] x DM to ${openId} failed: ${String(
          (err as any)?.message ?? err,
        )}`,
      );
      throw err;
