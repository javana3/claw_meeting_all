...
export class SlackProvider implements CalendarProvider {
  ...
  async sendTextDM(userId: string, text: string): Promise<string> {
    ...
    try {
      ...
      if (!res.ok) {
        console.error(`[slack.dm] x postMessage failed: ${res.error}`);
        throw new Error(`chat.postMessage failed: ${res.error}`);
      }

      const ts = res.ts ?? "";
      console.error(`[slack.dm] ok delivered ts=${ts}`);
      return ts;
    } catch (err: any) {
      console.error(`[slack.dm] x exception: ${err.message ?? err}`);
      throw err;
    }
  }
    ...
    for (const { query, userId } of directIds) {
      try {
        ...
        if (info.ok && info.user) {
          ...
          console.error(`[slack.resolve] ok user_id ${userId} -> ${info.user.real_name}`);
        } else {
          results.push({ query, userId: "", via: "unresolved" });
          console.error(`[slack.resolve] x user_id ${userId} -> not found`);
        }
      } catch (err: any) {
        results.push({ query, userId: "", via: "unresolved" });
        console.error(`[slack.resolve] x user_id ${userId} -> error: ${err.message}`);
      }
    }
    ...
    for (const email of emails) {
      try {
        ...
        if (res.ok && res.user) {
          ...
          console.error(`[slack.resolve] ok email ${email} -> ${res.user.id} (${res.user.real_name})`);
        } else {
          results.push({ query: email, userId: "", via: "unresolved" });
          console.error(`[slack.resolve] x email ${email} -> not found`);
        }
      } catch (err: any) {
        // Slack throws an error with data.error === "users_not_found" when no match
        if (err.data?.error === "users_not_found") {
          results.push({ query: email, userId: "", via: "unresolved" });
          console.error(`[slack.resolve] x email ${email} -> users_not_found`);
        } else {
          results.push({ query: email, userId: "", via: "unresolved" });
          console.error(`[slack.resolve] x email ${email} -> error: ${err.message}`);
        }
      }
    }
  ...
  private async fetchDirectory(): Promise<DirectoryCandidate[]> {
    ...
    do {
      ...
      console.error(
        `[slack.http] users.list page=${page} cursor=${cursor?.slice(0, 20) ?? "none"} ` +
          `status=${res.ok ? "ok" : res.error} elapsed=${elapsed}ms`
      );

      if (!res.ok || !res.members) {
        console.error(`[slack.walk] x users.list failed: ${res.error}`);
        break;
      }
    ...
    console.error(`[slack.walk] ok directory complete: ${candidates.length} users, ${page} pages`);
    return candidates;
  }
