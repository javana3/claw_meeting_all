/**
 * Meeting-scheduler skill core — MULTI-PLATFORM ROUTING (ESM Skill version).
 *
 * Exports `createMeetingPlugin(options)` which accepts MULTIPLE platform
 * configs. A single plugin instance registers 7 tools. On each tool call,
 * `ctx.messageChannel` determines which CalendarProvider to use.
 *
 * Differences from plugin version:
 *   - ESM imports with .js suffix
 *   - MeetingStore for persistent pending-meeting state
 *   - execute(toolCallId, params) signature
 *   - parameters cast as `as any`
 *   - No __dirname/require/module.exports at module scope
 */
...
import { CalendarProvider, ResolvedUser } from "./providers/types.js";
import {
  findCandidateSlots,
  findSlotsInWindows,
...
// ============================================================================
// PendingMeeting types & constants
// ============================================================================
...
function gcIdempotency(
  recentFindAndBook: Map<string, { meetingId: string; at: number }>,
  now: number,
): void {
...
function gcPending(
  pendingMeetings: Map<string, PendingMeeting>,
  now: number,
): void {
...
export function createMeetingPlugin(opts: MeetingPluginOptions | MeetingPluginSingleOptions) {
  ...
  const plugin = {
    ...
    register(api: any) {
      ...
      // --- Initialize MeetingStore for persistence ---
      const __filename = fileURLToPath(import.meta.url);
      const __dirname = path.dirname(__filename);
      const dataDir = path.resolve(__dirname, "..", "pending");
      const store = new MeetingStore(dataDir);
      store.hydrate();

      // Aliases for concise access
      const pendingMeetings = store.pendingMeetings;
      const recentFindAndBook = store.recentFindAndBook;
      const inflightFindAndBook = store.inflightFindAndBook;
      ...
      async function commitMeeting(m: PendingMeeting, slot: { start: Date; end: Date }): Promise<string> {
        ...
        if (!provider) { m.closed = true; store.save(m.id); return "No provider."; }
      ...
      async function finaliseMeeting(m: PendingMeeting): Promise<string> {
        const provider = providerForMeeting(m);
        if (!provider) { m.closed = true; store.save(m.id); return "No provider for this meeting's channel."; }
      ...
      api.registerTool((ctx: any) => {
        ...
        return {
          ...
          execute: async (_toolCallId: string, params: any) => {
            ...
            const now = Date.now();
            gcIdempotency(recentFindAndBook, now);
...
// ESM exports
// ============================================================================
export { createMeetingPlugin as default };
export type {
  AttendeeStatus,
  AttendeeResponse,
  MeetingPhase,
